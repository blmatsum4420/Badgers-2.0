...
def save_fig(fig1, fig2, name):
	fig1.savefig("myapp/static/plots/" + name.replace(' ','_') + '.jpg')
	fig2.savefig("myapp/static/plots/" + name.replace(' ','_') + '_trends' + '.jpg')
...
def make_fig(model, lookahead, title, ylabel, start=None, save=True):
	...
	print(title)
	fig1 = model.plot(forecast, xlabel='Date', ylabel=ylabel, figsize=(15,9))
	fig1.suptitle(title)
